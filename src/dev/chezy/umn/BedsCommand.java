                  ...
                  public void run(InventoryClickEvent e) {
                    if(e.getClick().isRightClick()) {
                      ...
                      if(UMN.inBedRename.containsKey(((Player)e.getWhoClicked()).getUniqueId().toString())) {
                        UMN.inBedRename.replace(((Player)e.getWhoClicked()).getUniqueId().toString(), bed.getLocation());
                        return;
                      }
                      ...
                      if(cooldown > 0) {
                        e.getWhoClicked().sendMessage(ChatColor.RED.toString() + ChatColor.translateAlternateColorCodes('&', bed.getName()) + ChatColor.RED.toString() + " is on cooldown for " + (cooldownMins > 0 ? cooldownMins + " minute" + (cooldownMins == 1 ? "" : "s") + " and " : "") + cooldownSecs + " second" + (cooldownSecs == 1 ? "" : "s") + "!");
                      } else {
                        e.getWhoClicked().closeInventory();
